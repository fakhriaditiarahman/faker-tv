...
// Android TV Key Codes (used by Infinix TV / Android TV)
const KEY_CODES = {
    // Power & Basic
    'KEY_POWER': 26,
    'KEY_HOME': 3,
...
class InfinixAdapter {
    constructor(io) {
        this.io = io;
        this.tvWs = null;
        this.httpPort = 8081;
        this.wsPort = 6466;
        this.tvConfig = { ip: null, mac: null, name: 'Infinix TV' };
    }
    ...
    connect(port = 6466) {
        ...
        console.log(`\n--- MENCOBA KONEKSI KE INFINIX TV (${this.tvConfig.ip}:${port}) ---`);
        ...
        try {
            ...
            this.tvWs.on('open', () => {
                console.log(`WebSocket Infinix Terbuka di Port ${port}!`);
            ...
            this.tvWs.on('message', (data) => {
                try {
                    ...
                    console.log('Infinix TV Response:', msg);
            ...
            this.tvWs.on('error', (err) => {
                ...
                // Fallback to port 64738 (common Android TV WebSocket port)
                if (port === 6466 && err.message.includes('ECONNREFUSED')) {
                    console.log('Mencoba fallback ke Port 64738...');
                    setTimeout(() => this.connect(64738), 2000);
                } else {
        ...
        if (!this.isConnected()) {
            ...
            setTimeout(() => {
                if (this.isConnected()) {
                    ...
                    console.log('Infinix TV masih belum siap, coba pencet lagi sebentar lagi bos.');
                }
            }, 3000);
            return;
        }
